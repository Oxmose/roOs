...
pre-run:
	...
	@cp -R Config/arch/x86_i386/GRUB ./$(KERNEL_BUILD_DIR)/
	@cp ./$(KERNEL_BUILD_DIR)/$(KERNEL).elf ./$(KERNEL_BUILD_DIR)/GRUB/boot/
	@grub-mkrescue -d ../Tools/i386-pc -o ./$(KERNEL_BUILD_DIR)/$(KERNEL)_boot.iso ./$(KERNEL_BUILD_DIR)/GRUB 2>&1 /dev/null

run: pre-run
	@echo "\e[1m\e[94m=== Running on Qemu\e[22m\e[39m"
	$(QEMU) $(QEMUOPTS) -boot d -cdrom ./$(KERNEL_BUILD_DIR)/$(KERNEL)_boot.iso -monitor telnet:127.0.0.1:55550,server,nowait
...
debug: pre-run
	...
	@$(QEMU) $(QEMUOPTS) -boot d -cdrom ./$(KERNEL_BUILD_DIR)/$(KERNEL)_boot.iso -S -monitor telnet:127.0.0.1:55550,server,nowait -gdb tcp::1234
