...
#define O_RDONLY VFS_PERM_READ

/** @brief Defines the VFS access permissions for read / write */
#define O_RDWR (VFS_PERM_READ | VFS_PERM_WRITE)
...
typedef struct
{
    /** @brief Driver's name, used with mount to detect the FS to use */
    char pName[FS_NAME_LENGTH + 1];
     ...
     */
    OS_RETURN_E (*pMount)(const char* kpPath,
...
#define VFS_REG_FS(DRIVER)                                                    \
    fs_driver_t* DRVENT_##DRIVER __attribute__ ((section (".roos_fs_tbl"))) = \
        &DRIVER;
